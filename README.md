...
This document outlines the steps taken to develop a comprehensive infrastructure plan for exposing the application via `kepler.notjustdevops.com` using Terragrunt(Deleted), while also implementing a CI/CD pipeline. It covers the Helm chart development process and the necessary configurations for AWS and Kubernetes.

## 1. Infrastructure Design (AWS + DNS)

### Domain Setup
To ensure proper domain management, `notjustdevops.com` is hosted in AWS Route 53, allowing control over DNS records for subdomains, including `kepler.notjustdevops.com`.

### Hosted Zone Creation
- A hosted zone for `kepler.notjustdevops.com` has been created or confirmed.
- DNS records for subdomains such as `argocd.kepler.notjustdevops.com` and `grafana.kepler.notjustdevops.com` have been established.

### SSL Certificates
Utilizing AWS Certificate Manager (ACM), SSL certificates have been obtained for:
- `kepler.notjustdevops.com`
- `argocd.kepler.notjustdevops.com`
- `grafana.kepler.notjustdevops.com`

### Ingress Configuration
An NGINX ingress controller has been set up to manage incoming traffic and handle SSL certificates via Let’s Encrypt or ACM.

## 2. Kubernetes Environment Setup

### EKS Cluster
The Elastic Kubernetes Service (EKS) cluster has been provisioned using Terragrunt, ensuring scalability and reliability for the application.

### Namespace Creation
Namespaces have been created within the Kubernetes cluster for better organization:
- `kepler` for the main application
- `argocd` for ArgoCD management
- `monitoring` for Grafana and Prometheus

## 3. Helm Chart Development

### Helm Chart Creation
A custom Helm chart was developed to streamline the deployment process. This includes:
- Templates for deployments, services, and ingress resources.
- A `values.yaml` file to configure settings for scaling, resource allocation, and environment-specific variables.

### Helm Repositories
The Helm chart is stored in a dedicated GitHub repository or a Helm Chart repository, enabling version control and easier distribution (As an Option)


## Steps to Complete:

1. **Set Up ArgoCD**: Use Helm to install ArgoCD in your EKS cluster, ensuring to configure all necessary RBAC settings.

2. **Configure Ingress**: Set up an Ingress controller (like NGINX or AWS Load Balancer Controller) to manage traffic. Define your Ingress resources in Helm charts or as separate YAML files, and set the `host` fields to the appropriate subdomains.

3. **Expose App**: Update your app's Helm chart to include an Ingress resource that routes traffic to your app's service.

4. **DNS Configuration**: In Route 53, create A records for `kepler.notjustdevops.com`, `grafana.kepler.notjustdevops.com`, and `argocd.kepler.notjustdevops.com`, pointing to the Ingress controller's load balancer.
